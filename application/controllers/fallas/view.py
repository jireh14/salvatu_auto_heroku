...
class View:
    ...
    def GET(self, codigo_falla):
        if app.session.loggedin is True: # validate if the user is logged
            ...
            if session_privilege == 0: # admin user
                return self.GET_VIEW(codigo_falla) # call GET_VIEW() function
    ...
    @staticmethod
    def GET_VIEW(codigo_falla):
        codigo_falla = config.check_secure_val(str(codigo_falla)) # HMAC id_falla validate
        result = config.model.get_fallas(codigo_falla) # search for the id_falla data
